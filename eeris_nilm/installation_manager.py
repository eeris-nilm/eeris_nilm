...
class InstallationManager(object):
    ...
    def on_delete_model(self, req, resp, inst_id):
        """
        Delete the stored model and start again. Useful for testing, or when the model
        needs to be rebuilt from scratch from the data.
        CAUTION: This can cause problems if this route is called when the service is in
        production. It will not delete model unless the 'debugInstallation' field is set
        to True.
        """
        ...
        if model is None:
            ...
            if debug:
                # There should be only one document. In any case, delete only one.
